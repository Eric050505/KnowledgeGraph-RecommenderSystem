...
class TransE(torch.nn.Module):
    ...
    def forward(self, head, rel, tail) -> torch.Tensor:
        head_tensor = torch.IntTensor(head).to(self.device)
        tail_tensor = torch.IntTensor(tail).to(self.device)

        if max(head) >= self.ent_num or max(tail) >= self.ent_num:
            default_emb = torch.mean(self.ent_embedding.weight, dim=0).to(self.device)
            head_mask = head_tensor < self.ent_num
            tail_mask = tail_tensor < self.ent_num

            head_emb = torch.where(
                head_mask.unsqueeze(-1).expand(-1, self.ent_embedding.embedding_dim),
                self.ent_embedding(torch.clamp(head_tensor, 0, self.ent_num - 1)),
                default_emb
            )
            tail_emb = torch.where(
                tail_mask.unsqueeze(-1).expand(-1, self.ent_embedding.embedding_dim),
                self.ent_embedding(torch.clamp(tail_tensor, 0, self.ent_num - 1)),
                default_emb
            )
        else:
            head_emb = self.ent_embedding(head_tensor)
            tail_emb = self.ent_embedding(tail_tensor)
...
class KGRS:
    def __init__(self, train_pos: np.array, train_neg: np.array, kg_lines: List[str]):
        ...
        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2.5, "emb_dim": 128, "l1": True, "margin": 80,
                  "learning_rate": 1e-3, "weight_decay": 2e-3, "epoch_num": 40}
        self.config = config
